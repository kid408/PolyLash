extends SkillQBase
class_name SkillQuartermasterQ
...
func _get_q_asset_kind(is_closed_path: bool) -> String:
	return "quartermaster_supply_hub" if is_closed_path else "quartermaster_supply_link"

func _get_q_asset_duration(is_closed_path: bool) -> float:
	return max(buff_duration, 1.0) if is_closed_path else _get_line_duration()
...
func _build_q_asset_payload(
	...
	return {
		...
		"center": center,
		"radius": radius,
		"supply_attack_boost": supply_attack_boost,
		"supply_cooldown_reduction": supply_cooldown_reduction,
		"points": path_points.duplicate(),
		"path_start": path_points[0] if path_points.size() >= 1 else center,
		"path_end": path_points[path_points.size() - 1] if path_points.size() >= 1 else center,
	}
