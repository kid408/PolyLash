...
var saw_launch_distance_cap: float = 340.0
...
func _launch_saw_construct() -> void:
	...
	print("[SkillSawPath] 鍙戝皠璺濈: %.1f (saw_max_distance=%.1f, cap=%.1f)" % [launch_distance, saw_max_distance, saw_launch_distance_cap])
	...
	if skill_owner:
		...
		saw.setup(path_points, is_path_closed, fly_dir, skill_owner, launch_distance, chain_radius)
