...
const ROLE_ID: String = "new_totem"
...
func _apply_q_link_signature(phase: String, packet: Dictionary, center: Vector2) -> void:
	...
	if q_closed:
		_on_new_totem_pulse_timeout(q_center, q_radius * 0.72, 0.30, true)
		get_tree().create_timer(0.18).timeout.connect(
			_on_new_totem_pulse_timeout.bind(q_center + q_dir * (q_radius * 0.22), q_radius * 0.66, 0.24, false)
		)
		_line_slice_burst(
		...
		if _is_totem_window():
			_on_new_totem_pulse_timeout(q_center, q_radius * 0.60, 0.26, true)
	else:
		_on_new_totem_pulse_timeout(q_center, q_radius * 0.50, 0.18, false)
		_line_slice_burst(
		...
		if _is_totem_window():
			get_tree().create_timer(0.12).timeout.connect(
				_on_new_totem_pulse_timeout.bind(q_center, q_radius * 0.46, 0.16, false)
			)
...
func _role_signature(phase: String, packet: Dictionary, center: Vector2) -> void:
	...
	if phase == "line":
		_on_new_totem_pulse_timeout(center, radius * 0.52, 0.18, false)
		_line_slice_burst(
		...
		_on_new_totem_pulse_timeout(center, radius * 0.74, 0.28, true)
		get_tree().create_timer(0.20).timeout.connect(
			_on_new_totem_pulse_timeout.bind(center + side_dir * (radius * 0.26), radius * 0.64, 0.22, false)
		)
		_schedule_line_sweep_sequence(
			center,
			aim_dir,
