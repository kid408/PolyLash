...
func _apply_q_link_signature(phase: String, packet: Dictionary, center: Vector2) -> void:
	...
	if q_closed:
		...
		_emit_tesla_grid(q_center, q_dir, q_radius * 1.22, true)
...
func _role_signature(phase: String, packet: Dictionary, center: Vector2) -> void:
	...
	if phase == "line":
		_emit_tesla_grid(center, tesla_dir, radius * 1.26, false)
		...
		_emit_tesla_grid(center, tesla_dir, radius * 1.46, true)
		_schedule_line_sweep_sequence(
			center,
			tesla_dir,
			radius * 1.55,
...
func _emit_tesla_grid(center: Vector2, aim_dir: Vector2, length: float, closure_phase: bool) -> void:
