...
func _apply_q_link_signature(phase: String, packet: Dictionary, center: Vector2) -> void:
	...
	if q_closed or phase == "closure":
		...
		if target != null:
			...
			get_tree().create_timer(0.12).timeout.connect(_on_hunter_shot_timeout.bind(weakref(target), 0.52))
...
func _role_signature(phase: String, packet: Dictionary, center: Vector2) -> void:
	...
	var hunter_side: Vector2 = Vector2(-hunter_dir.y, hunter_dir.x)
	if phase == "line":
		_emit_crosshair_lance(center, hunter_dir, radius * 1.10, false)
		_schedule_line_sweep_sequence(
			center,
			hunter_dir,
			radius * 1.45,
		...
		_emit_crosshair_lance(center, hunter_dir, radius * 1.32, true)
		_schedule_line_sweep_sequence(
			center,
			hunter_dir,
			radius * 1.6,
