...
func _apply_q_link_signature(phase: String, packet: Dictionary, center: Vector2) -> void:
	...
	if q_closed:
		_spawn_goo_pool(q_center, q_radius * 0.34, 1.8, 0.26, 2)
		_spawn_goo_pool(q_center + q_side * (q_radius * 0.26), q_radius * 0.26, 1.4, 0.22, 1)
		_line_slice_burst(
		...
		_spawn_goo_pool(q_center, q_radius * 0.22, 1.1, 0.16, 0)
		_line_slice_burst(
...
func _role_signature(phase: String, packet: Dictionary, center: Vector2) -> void:
	...
	if devour_active:
		...
		_spawn_goo_pool(center, radius * 0.24, 1.4, 0.22, 1)
	if phase == "line":
		_spawn_goo_pool(center + aim_dir * (radius * 0.24), radius * 0.22, 1.2, 0.16, 1)
		_line_slice_burst(
		...
		if devour_active:
			_spawn_goo_pool(center - aim_dir * (radius * 0.20), radius * 0.20, 1.1, 0.16, 0)
	elif phase == "closure":
		var closure_split: int = 2 + min(2, max(0, split_bonus - 2))
		_spawn_goo_pool(center, radius * 0.30, 1.7, 0.24, closure_split)
		_spawn_goo_pool(center + aim_dir * (radius * 0.28), radius * 0.24, 1.4, 0.20, 1 + min(1, split_bonus / 3))
