...
func _apply_q_link_signature(phase: String, packet: Dictionary, center: Vector2) -> void:
	...
	if q_closed or phase == "closure":
		...
		_emit_gildhand_coin_rail(q_center, q_dir, q_radius * 1.16, true)
		_line_slice_burst(
...
func _role_signature(phase: String, packet: Dictionary, center: Vector2) -> void:
	_signature_gildhand(phase, packet, center)
	...
	if phase == "line":
		_emit_gildhand_coin_rail(center, aim_dir, radius * 1.02, false)
		...
		_emit_gildhand_coin_rail(center, aim_dir, radius * 1.24, true)
...
func _emit_gildhand_coin_rail(center: Vector2, aim_dir: Vector2, length: float, closure_phase: bool) -> void:
