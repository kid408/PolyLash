...
const ROLE_ID: String = "runeblazer"
...
func _apply_mode_signature(phase: String, packet: Dictionary, center: Vector2, _hit_count: int) -> void:
	...
	_spawn_runeblazer_pickups(phase, packet, center)
...
func _role_signature(phase: String, packet: Dictionary, center: Vector2) -> void:
	_signature_runeblazer(phase, packet, center)
...
func _spawn_runeblazer_pickups(phase: String, packet: Dictionary, center: Vector2) -> void:
	...
	_spawn_signature_pickup_burst(
		...
		count,
		radius,
		Color(1.0, 0.54, 0.16, 0.96),
		{
			"effect_id": "runeblazer_rune",
			"pickup_text": "符炎页",
			"radius": radius,
			"text_color": Color(1.0, 0.62, 0.22),
			"vfx_color": Color(1.0, 0.46, 0.12, 0.90),
			"effect_scale": 0.58,
		},
