...
const ROLE_ID: String = "singularist"
...
func _apply_mode_signature(phase: String, packet: Dictionary, center: Vector2, _hit_count: int) -> void:
	...
	_spawn_singularist_pickups(phase, packet, center)
...
func _role_signature(phase: String, packet: Dictionary, center: Vector2) -> void:
	_signature_singularist(phase, packet, center)
...
func _spawn_singularist_pickups(phase: String, packet: Dictionary, center: Vector2) -> void:
	var radius: float = max(60.0, float(packet.get("radius", 140.0)) * 0.62)
	var count: int = 1 if phase != "closure" else 2
	_spawn_signature_pickup_burst(
		center,
		count,
		radius,
		Color(0.78, 0.98, 1.0, 0.95),
		{
			"effect_id": "singularist_dust",
			"pickup_text": "尘核",
			"radius": radius,
			"text_color": Color(0.82, 1.0, 1.0),
			"vfx_color": Color(0.68, 0.95, 1.0, 0.92),
			"effect_scale": 0.60,
		},
