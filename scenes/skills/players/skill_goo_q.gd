...
var goo_lance_step_distance: float = 52.0
var goo_lance_tick_interval: float = 0.05
var goo_lance_hit_radius: float = 52.0
var goo_lance_damage: int = 14
var goo_lance_push: float = 14.0
var goo_recall_delay: float = 0.2
var goo_recall_count: int = 5
var goo_recall_damage: int = 18
var goo_recall_pull: float = 20.0
var goo_reel_count: int = 5
var goo_reel_interval: float = 0.16
var goo_reel_damage: int = 18
...
func _spawn_area_effect(polygon: PackedVector2Array) -> void:
	...
	_spawn_goo_reel(polygon)
	_cache_goo_window(polygon, pool_duration)

func _launch_goo_lance(start: Vector2, finish: Vector2) -> void:
	...
	var step_total: int = int(max(2.0, ceil(length / max(12.0, goo_lance_step_distance))))
	var step_index: int = 0
	var timer: Timer = Timer.new()
	timer.wait_time = max(0.03, goo_lance_tick_interval)
	...
	timer.timeout.connect(func() -> void:
		...
		if step_index > step_total:
			timer.stop()
			host.queue_free()
			_spawn_goo_recall(finish, start, move_dir)
			return
		_emit_goo_lance_tick(start, finish, step_index, step_total, move_dir)
		step_index += 1
	)
	timer.start()

func _spawn_goo_recall(from_pos: Vector2, to_pos: Vector2, move_dir: Vector2) -> void:
	var tree: SceneTree = get_tree()
	if tree == null:
		return
	tree.create_timer(max(0.05, goo_recall_delay)).timeout.connect(func() -> void:
		if not is_inside_tree():
			return
		var host: Node2D = Node2D.new()
		host.name = "GooRecallHost"
		add_child(host)
		var pulse_total: int = int(max(1, goo_recall_count))
		var pulse_index: int = 0
		var timer: Timer = Timer.new()
		timer.wait_time = max(0.06, goo_lance_tick_interval * 1.6)
		timer.one_shot = false
		host.add_child(timer)
		timer.timeout.connect(func() -> void:
			if not is_instance_valid(host):
			...
			if pulse_index >= pulse_total:
				...
				host.queue_free()
				return
			_emit_goo_recall_tick(from_pos, to_pos, pulse_index, pulse_total, move_dir)
			pulse_index += 1
		)
		timer.start()
	)

func _emit_goo_lance_tick(start: Vector2, finish: Vector2, index: int, total: int, move_dir: Vector2) -> void:
	...
	_apply_radius_goo_damage(current, goo_lance_hit_radius, goo_lance_damage)
	_apply_direction_push(current, goo_lance_hit_radius * 1.2, move_dir, goo_lance_push)

func _emit_goo_recall_tick(
	from_pos: Vector2,
	to_pos: Vector2,
	index: int,
	total: int,
	...
	_apply_line_burst_damage(start, end, 11.0, goo_recall_damage)
	_apply_pull_to_point(center, goo_lance_hit_radius * 1.55, goo_recall_pull)

func _spawn_goo_reel(polygon: PackedVector2Array) -> void:
	var center: Vector2 = _calculate_polygon_center(polygon)
	var radius: float = _polygon_radius(polygon, center)
	var sweep_total: int = int(max(1, goo_reel_count))
	...
	var timer: Timer = Timer.new()
	timer.wait_time = max(0.08, goo_reel_interval)
	timer.one_shot = false
	host.add_child(timer)
	timer.timeout.connect(func() -> void:
		if not is_instance_valid(host):
		...
		if sweep_index >= sweep_total:
			...
			host.queue_free()
			return
		_emit_goo_reel(center, radius, sweep_index, sweep_total)
		sweep_index += 1
	)
	timer.start()

func _emit_goo_reel(center: Vector2, radius: float, index: int, total: int) -> void:
	...
	_apply_line_burst_damage(start, end, 11.0, goo_reel_damage)
	_apply_pull_to_point(center, radius * 1.02, goo_recall_pull * 0.55)
...
func _cache_goo_window(polygon: PackedVector2Array, duration: float) -> void:
...
func _apply_line_burst_damage(start: Vector2, end: Vector2, hit_radius: float, damage: int) -> void:
	...
	for enemy_obj: Variant in enemies:
		...
		_apply_status(enemy, "poison", 1.2, goo_poison_value, 1, 0.7)

func _apply_radius_goo_damage(center: Vector2, radius: float, damage: int) -> void:
	var enemies: Array = get_tree().get_nodes_in_group("enemies")
	for enemy_obj: Variant in enemies:
		if enemy_obj == null or not is_instance_valid(enemy_obj):
			continue
		...
		if enemy.global_position.distance_to(center) > radius:
			continue
		_apply_damage(enemy, damage)
		_apply_status(enemy, "poison", 1.0, goo_poison_value, 1, 0.7)
