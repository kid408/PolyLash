...
func execute() -> void:
	...
	var bad_streak: int = int(skill_owner.get_meta("fatebinder_e_bad_streak", 0))
	...
	if roll < jackpot_rate:
		skill_owner.set_meta("fatebinder_e_bad_streak", 0)
	...
	if roll < jackpot_rate + stable_rate:
		skill_owner.set_meta("fatebinder_e_bad_streak", bad_streak + 1)
