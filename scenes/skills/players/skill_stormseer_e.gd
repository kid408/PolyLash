...
func execute() -> void:
	...
	var window_data: Array = _get_stormseer_window(center, final_radius)
...
func _get_stormseer_window(default_center: Vector2, default_radius: float) -> Array:
	var data: Array = [false, default_center, default_radius]
	if not is_instance_valid(skill_owner):
		return data
	if not skill_owner.has_meta(STORMSEER_META_EXPIRE_MSEC):
		return data
	var expire_msec: int = int(skill_owner.get_meta(STORMSEER_META_EXPIRE_MSEC, 0))
	if Time.get_ticks_msec() > expire_msec:
		return data
	var center_val: Variant = skill_owner.get_meta(STORMSEER_META_CENTER, default_center)
	var radius_val: Variant = skill_owner.get_meta(STORMSEER_META_RADIUS, default_radius)
