...
func _spawn_area_effect(polygon: PackedVector2Array) -> void:
	...
	_cache_tempest_window(polygon, area_duration)
...
func _cache_tempest_window(polygon: PackedVector2Array, duration: float) -> void:
	if not is_instance_valid(skill_owner):
		return
	var center: Vector2 = _calculate_polygon_center(polygon)
	var radius: float = _polygon_radius(polygon, center)
	var expire_msec: int = Time.get_ticks_msec() + int(round(max(0.2, duration) * 1000.0))
	skill_owner.set_meta(TEMPEST_META_CENTER, center)
	skill_owner.set_meta(TEMPEST_META_RADIUS, radius)
	skill_owner.set_meta(TEMPEST_META_EXPIRE_MSEC, expire_msec)
