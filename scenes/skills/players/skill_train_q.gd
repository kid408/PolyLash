...
func _spawn_line_effect(start: Vector2, end: Vector2) -> void:
	_spawn_train_pass(start, end, 0.12, rail_width, dash_damage)
	_spawn_train_pass(end, start, 0.44, rail_width * 0.82, back_damage)
...
func _spawn_train_pass(start: Vector2, end: Vector2, delay_sec: float, width: float, damage: int) -> void:
	var tree: SceneTree = get_tree()
	if tree == null:
		return
