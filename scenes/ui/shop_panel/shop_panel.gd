...
func _ready() -> void:
	...
	ShopManager.shop_item_updated.connect(_on_shop_item_updated)
...
func show_shop(wave_number: int) -> void:
	...
	ShopManager.generate_shop_items(4, wave_number)
...
func _update_gold_display() -> void:
	...
	reroll_button.text = "刷新商店（%d 金币）" % reroll_cost
...
func _create_item_cards(items: Array) -> void:
	...
	for i in range(items.size()):
		var item_variant: Variant = items[i]
		...
		card.replace_target_selected.connect(_on_replace_target_selected)
...
func _on_shop_item_updated(index: int, item: Dictionary) -> void:
	if index < 0 or index >= item_cards.size():
		return
	var card: ShopItemCard = item_cards[index]
	if is_instance_valid(card):
		card.refresh(item)
	_update_gold_display()
...
func _on_replace_target_selected(card_index: int, replace_out_player_id: String) -> void:
	ShopManager.update_recruit_replace_target(card_index, replace_out_player_id)
