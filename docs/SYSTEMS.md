...
角色选择与强化系统是游戏的准备阶段，玩家在此选择角色、武器并强化属性。

详细文档请参考: [SELECTION_SYSTEM.md](SELECTION_SYSTEM.md)

### 主要功能

1. **角色选择界面** (`selection_panel`)
   - 选择最多3个角色组成队伍
   - 为每个角色选择初始武器
   - 查看角色详细信息

2. **角色强化界面** (`character_upgrade`)
   - 使用金币升级角色属性（生命值、能量、速度等）
   - 随机武器商店（Roguelike风格）
   - 每个属性最多升级5级
...
| 配置项 | 说明 |
|--------|------|
| max_selected_players | 最多可选角色数量 |
| max_upgrade_level | 属性最大升级等级 |
| reset_attributes_on_new_game | 新游戏是否重置属性 |
| enable_starting_weapon_shop | 是否启用随机武器商店 |
...
更多详细信息请查看其他文档：
- [SELECTION_SYSTEM.md](SELECTION_SYSTEM.md) - 角色选择与强化系统
